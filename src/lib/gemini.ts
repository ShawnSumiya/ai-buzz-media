...
- 短文中心。1文が長くなりすぎるな
- スペックの説明禁止。論理的な説明はダメ。体験・感情だけ語れ
- 「〜なので便利です」みたいな論理構文禁止。「これ神」「マジでヤバい」など直感的に
- 適度に誤字、「w」「（笑）」「！」の連打を混ぜてリアリティを出す

【商品名の使い方】
- 商品の正式名称・型番の連呼は禁止。スレッド冒頭や前の会話で既に商品が分かっているなら「これ」「あれ」「それ」「〇〇のやつ」など自然な代名詞を使う
- 全員が商品名を復唱するのは業者のサクラっぽく不自然。スペック・価格も全員が言わない。分散させる
