...
【★会話構造の指定（商品名の連呼禁止）★】
- スレッドの最初（>>1相当）：必ず略称・通称を用いて、何について話すかを明示すること。
...
【★会話構造の指定（商品名の連呼禁止）★】
- 既存会話（>>1相当）で既に略称・通称が提示されている場合、追いコメントでは「それ」「あれ」「あのブランドのやつ」などの代名詞を使用するか、主語を省略すること。
- 全員が律儀に商品名から会話を始める不自然な挙動を禁止する。
...
【★会話構造の指定（商品名の連呼禁止）★】
- 既存会話で既に略称・通称が提示されている場合、続きコメントでは「それ」「あれ」「あのブランドのやつ」などの代名詞を使用するか、主語を省略すること。
- 全員が律儀に商品名から会話を始める不自然な挙動を禁止する。
